...
- **`--snapshot-sync`**  
  *Description*: Start a node downloading from a data snapshot.  
  *Default*: `false`  
  *Usage*: `--snapshot-sync` 

- **`--snapshot-sync-command`**  
  *Description*: Specify a unique command to run a snapshot-sync startup, including download command, creating/moving directories, and more. All occurs before the node starts up. 
  *Usage*: `--snapshot-sync-command="<command>"`  
  *Example*: `--snapshot-sync-command="..."`  

### Nodevin Data Location

- **`--data-dir`**  
  *Description*: Location for nodevin and blockchain data on disk.  
...
- **`nodevin delete <network>`**  
  *Description*: Deletes local blockchain data associated with a specific network.  
  *Usage*: `nodevin delete <network>`  
  *Example*: `nodevin delete bitcoin`  
  - **`delete all`**  
    *Description*: Deletes all nodevin data (the entire Nodevin data directory).  
    *Usage*: `nodevin delete all`  
